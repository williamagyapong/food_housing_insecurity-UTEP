...
knitr::opts_chunk$set(echo = F, warning=F, message=F, comment = F, fig.pos = "!H", out.extra = "")
...
plot(dat$ensemble, main="Performance of individual models versus the Ensemble model")
...
rank_pred_by_ensemb(dat$ensemble, 20) + rank_pred(dat$model_list$glm, 20)
...
```{r}
rank_pred(dat$model_list$svmRadial, 20, "MARS")
# rank_pred_by_ensemb(dat$ensemble, 20)

# The ensemble model performed poorly, so we used to MARS model and the SVM Radial models to
...
eval_table(dat$model_list, "FI_q26", "Food Insecurity I")
...
eval_table(dat$model_list, "FI_q27", "Food Insecurity II")
...
rank_pred_by_ensemb(dat$ensemble, 20) + rank_pred(dat$model_list$earth)
...
eval_table(dat$model_list, "FI_q28", "Food Insecurity III")
...
eval_table(dat$model_list, "FI_q30", "Food Insecurity IV")
...
eval_table(dat$model_list, "FI_q31", "Food Insecurity V")
...
rank_pred(dat$model_list$glm, 20)
