...
data_pa<- data %>%
  ...
  dplyr::select(-starts_with("FI_"), -ends_with("_changed")) %>%
...
data_pad<-cbind("spent_night_elsewhere"=data_pa$spent_night_elsewhere,data1)
...
```{r}
library(ROSE)
 data_pad_balance<-ovun.sample(spent_night_elsewhere ~ ., data = data_pad, method = "both", p=0.5,                             N=NROW(data_pad), seed = 125)$data
