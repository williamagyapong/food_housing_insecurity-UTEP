...
data_pa<- data %>%
  ...
  dplyr::select(-c(permanent_address,spent_night_elsewhere), -ends_with("_changed")) %>%
...
data_imputed <- mice(data_pa[,-20], printFlag = F)
...
#     misscal<- round(mean(pred.test != testing$FI_q28),digits = 2)
#     
#    #  test_pred_1 <- predict(model, newdata = testing, type= "prob")
#    #  ROC_SR <- roc(testing$FI_q28, predictor = test_pred_1[,2])
