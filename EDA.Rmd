...
eda_dat <- FIHI_sub3 %>%
  select(-ends_with("_changed"), -respondent_id)
...
# - create a count column after fixing multiple response issues to help 
...
# - 
# - spent night elsewhere (Q22): filter out 'Yes' for permanent address
# - 

summ_stats <- data.frame() # initialize summary statistics container
nc <- NCOL(eda_dat)
var_name <- variable.names(eda_dat)
nr_vec <- vector(length = nc) # to store number of rows for the summary df of each variable
dd <- c("employment_type"="No", "weekly_work_hrs"="No", "transport_reliability"="Not applicable", "spent_night_elsewhere"="Yes")
aa <- c("employment_type"="employment", "weekly_work_hrs"="employment", "transport_reliability"="mode_transport", "spent_night_elsewhere"="permanent_address")

for (i in 1:nc) {

  # Computing the summary statistics for the ith variable
  
  if(var_name[i] %in% names(dd)) {
    summ <- eda_dat %>% 
              filter(!!as.name(aa[var_name[i]]) != dd[var_name[i]]) %>%
  ...
  summ_stats <- rbind(summ_stats, summ, stringsAsFactors = F)
  nr_vec[i] <-  length(var_col)
}
...
get_stripe_index <- function(indices)
{
  ...
  for(i in seq_along(indices)) {
    ...
    begin <- end - (indices[i]-1)
    
    index_vec <- c(index_vec, begin:end)
...
kable(summ_stats, align = "llll", longtable=T, booktabs=T, linesep="",
 ...
 # kable(sum_stats, caption = "Summary statistics for variables of interest",format = "latex", booktabs=T, linesep="") %>% 
 #    kable_styling(full_width = F, font_size = 11,latex_options = c("HOLD_position", "striped"),stripe_index = c(1:nrow(pa_stats))) 
