...
## classification through logistics  regression
```{r}
# Training model
logistic_model <- glm(permanent_address ~., 
                      data = training, 
                      family = "binomial")
#logistic_model
```
...
predict_reg <- predict(logistic_model,newdata = testing, type = "response")
# Changing probabilities
#predict_reg <- ifelse(predict_reg >0.5, 1, 2)
# using confusion matrix
table(testing$permanent_address, predict_reg)
missing_classer <- mean(predict_reg != testing$permanent_address)
print(paste('Accuracy =', 1 - missing_classer))
```

```{r}
# ROC-AUC Curve
ROCPred <- prediction(predict_reg, testing$permanent_address) 
...
set.seed(123)
model_list <- caretList(permanent_address ~., data = training,
                        trControl = my_control,
                        methodList = c("svmRadial", "rf", "glm","knn", "sda", "pls","nnet",
                                       "lda","nb"),
                        tuneList = NULL,
...
```{r, echo=F, message=FALSE}
# Obtaining Misclassification 
model_Radial <- model_list$svmRadial
pred.test_rad <- model_Radial %>% predict(testing)
conf_mat_R<-confusionMatrix(pred.test_rad, testing$permanent_address)
tab_R<- conf_mat_R$table
misscal_R<- round((1-sum(diag(tab_R)/sum(tab_R)))*100,digits = 2)
...
```{r, echo=F, message=FALSE}
test_pred_R <- predict(model_Radial, newdata = testing, type= "prob")
ROC_SR <- roc(response=testing$permanent_address, predictor = factor(test_pred_R[, 2]))
plot(ROC_SR, col="brown")
AUC_R<-round(ROC_SR$auc, digits=4)  # AUC
text(x=0.4, y=0.25, paste("Area Under Curve = ", AUC_R, sep=""), col="blue", cex=1.2) 
```
